...
def generate_products(num_products=30):
    ...
    for a_thing in range(num_products):
        The_product = sample(ADJECTIVES, 1)[0] + '' + sample(NOUNS, 1)[0]
        price = randint(5, 100)
        weight = randint(5, 100)
        flannability = uniform(0.0, 2.5)
        products.append(Product(The_product, price=price,
                                weight=weight, flannability=flannability))
...
def inventory_report(products):
    ...
    for another_thing in products:
        unique_names.add(another_thing.name)
    ...
    print(a_price/len(products))
    print('Average weight:')
    print(a_weight/len(products))
    print('Average flammability:')
    print(a_flannability/len(products))
